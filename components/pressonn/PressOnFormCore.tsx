...
// para acelerar la subida y respetar el límite de 6MB del bucket.
...
// Formulario de Press-On embebido en la página /press-on (sin modal/overlay).
export function PressOnFormCore() {
  ...
  function resetForm() {
    ...
    setMedidas((prev) => {
      Object.values(prev).forEach((p) => p && URL.revokeObjectURL(p.url));
      return {};
    });
    setNombre("");
    setWhatsapp("");
  ...
  if (done) return <SuccessView nombre={nombre} onReset={resetForm} />;

  return (
    <div className="rounded-3xl border border-violet-100 bg-white p-5 shadow-[0_8px_28px_rgba(123,92,255,.08)]">
      <p className="text-sm leading-relaxed text-ink-500">
        Cuéntanos cómo las quieres y te preparamos un kit personalizado. Con esto armamos tu
        presupuesto y te escribimos por WhatsApp con los colores y el pago. 💜
      </p>
      ...
      <PhotoField label="Foto de referencia" required pic={ref} onPick={pickReferencia} hint="El diseño que quieres lograr" disabled={submitting} />
      ...
      <div className="mb-3 rounded-2xl border border-violet-200 bg-violet-50/60 p-3">
        <div className="flex gap-3">
          ...
          <div className="min-w-0 text-[11px] leading-relaxed text-ink-600">
            ...
            <p className="mt-1">
              En cada foto coloca una <strong>tapa de plástico de refresco</strong> cerca de la
              mano. Nos sirve de referencia de tamaño y es <strong>obligatoria</strong>.
            </p>
      ...
      <div className="grid grid-cols-2 gap-2">
        {MEDIDAS.map((m) => (
          <PhotoField key={m.key} label={m.label} compact pic={medidas[m.key] ?? null} onPick={(f) => pickMedida(m.key, f)} disabled={submitting} />
        ))}
      </div>
      ...
      {entrega === "envio" && (
        <div className="mt-3 space-y-3">
          ...
          {agencia && (
            <TextField label={`Dirección de la agencia ${labelOf(AGENCIAS, agencia)}`} value={direccionEnvio} onChange={setDireccionEnvio} placeholder="Ciudad, sucursal/oficina, punto de referencia…" maxLength={200} disabled={submitting} />
          )}
      ...
      <p className="mt-3 text-[11px] leading-relaxed text-ink-500">
        Te escribiremos por WhatsApp con el presupuesto a tasa BCV, los colores disponibles y los
        datos de pago para confirmar tu pedido.
      </p>
...
function SuccessView({ nombre, onReset }: { nombre: string; onReset: () => void }) {
  const waText = `Hola! Soy ${nombre || "una clienta"} 💜 Acabo de enviar mi solicitud de kit Press-On por la web.`;
  return (
    <div className="rounded-3xl border border-violet-100 bg-white px-6 py-10 text-center shadow-[0_8px_28px_rgba(123,92,255,.12)]">
      ...
      <button onClick={onReset} className="mt-3 inline-flex w-full items-center justify-center rounded-full bg-violet-50 px-6 py-3 text-xs font-bold text-violet-700">
